...
import com.oichkatzelesfrettschen.keenclassik.data.ValidationError
...
fun buildCellAccessibilityDescription(
    ...
    if (ValidationError.hasAnyError(errorFlags)) {
        val errorParts = mutableListOf<String>()
        if (ValidationError.hasRowError(errorFlags)) errorParts.add("row conflict")
        if (ValidationError.hasColumnError(errorFlags)) errorParts.add("column conflict")
        if (ValidationError.hasCageError(errorFlags)) errorParts.add("cage conflict")
        parts.add("error: ${errorParts.joinToString(" and ")}")
    }
...
/**
 * Format cell value for speech.
 */
private fun formatValueForSpeech(value: Int): String = value.toString()
