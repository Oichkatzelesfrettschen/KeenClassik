...
import com.oichkatzelesfrettschen.keenclassik.perf.PerfTrace
 ...
 */
object PuzzleParser {
    private const val MAX_CLUE_VALUE = 9999
    ...
    fun parse(payload: String, size: Int): ParseResult {
        return PerfTrace.section("PuzzleParser.parse") {
            ...
            // Parse zone definitions (6 chars each: operation char + 5-digit value, capped at 4 digits)
            ...
            for (i in 0 until uniqueZones) {
                ...
                if (value > MAX_CLUE_VALUE) {
                    return@section ParseResult.Failure(
                        "Zone value exceeds cap: $valueStr",
                        semicolonIndex + 1 + pos + 1
                    )
                }
