/*
 * KeenModel.java: Core game state model for Keen puzzles
 *
...
public class KeenModel {
    ...
    public KeenModel(int size, Zone[] zones, GridCell[][] grid, int[] dsf, long[] clues)
    {
        this.size = size;
        this.gameGrid = grid;
        this.undo = new Stack<>();
    ...
    /** DSF array for cage membership (row-major, root indices). */
    public int[] getDsf() { return dsf; }

    /** Cage clues with operation bits encoded in upper bits (row-major, root indices). */
    ...
    public void ensureInitialized() {
        ...
        ensureValidationData();
    }

    private void ensureValidationData() {
        if (dsf != null && clues != null) {
            return;
        }
        int n = size * size;
        dsf = new int[n];
        ...
        int[] zoneRoots = new int[gameZones.length];
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                GridCell cell = gameGrid[x][y];
                int zoneId = cell.zone.code;
                int idx = x * size + y;
                if (zoneRoots[zoneId] == -1) {
                    zoneRoots[zoneId] = idx;
                }
                dsf[idx] = zoneRoots[zoneId];
            }
        }
        ...
        for (int i = 0; i < gameZones.length; i++) {
            ...
            if (root >= 0 && root < clues.length) {
                clues[root] = encodeClue(gameZones[i]);
            }
        }
    }

    private long encodeClue(Zone zone) {
        long op;
        ...
        return op | (long) zone.expectedValue;
    }
