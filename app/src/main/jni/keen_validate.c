...
static int clue_matches(const validate_ctx* ctx, uint64_t result, uint64_t target) {
    if (!HAS_MODE(ctx->mode_flags, MODE_MODULAR) || ctx->w <= 0) {
        return result == target;
    }
    return (result % (uint64_t)ctx->w) == target;
}
...
static int check_cage_constraint(const validate_ctx* ctx, int root, digit* values, int ncells) {
    ...
    switch (op) {
        case C_ADD: {
            ...
            return clue_matches(ctx, sum, target);
        }

        case C_MUL: {
            uint64_t prod = 1;
            for (i = 0; i < ncells; i++) prod *= (uint64_t)values[i];
            return clue_matches(ctx, prod, target);
        }
        ...
        case C_SUB: {
            ...
            return clue_matches(ctx, diff, target);
        }
        ...
        case C_DIV: {
            ...
            if (b == 0) return 0;
            if (a % b != 0) return 0;
            return clue_matches(ctx, a / b, target);
        }
        ...
        case C_EXP: {
            ...
            for (i = 0; i < (int)b; i++) {
                pow_ab *= a;
                if (!modular && pow_ab > target) break;
            }
            for (i = 0; i < (int)a; i++) {
                pow_ba *= b;
                if (!modular && pow_ba > target) break;
            }
            return clue_matches(ctx, pow_ab, target) || clue_matches(ctx, pow_ba, target);
        }
        ...
        case C_GCD: {
            ...
            return clue_matches(ctx, (uint64_t)g, target);
        }
        ...
        case C_LCM: {
            ...
            return clue_matches(ctx, (uint64_t)l, target);
        }
        ...
        case C_XOR: {
            ...
            return clue_matches(ctx, x, target);
        }
