...
class JniResultParserTest {
    ...
    fun `parse ERR with invalid params code`() {
        val result = JniResultParser.parse("ERR:4:Size must be 3-9 for Classik profiles")
        assertTrue(result is JniResult.Error)

        val error = result as JniResult.Error
        assertEquals(JniErrorCode.INVALID_PARAMS, error.errorCode)
        assertEquals("Size must be 3-9 for Classik profiles", error.message)
    }

    @Test
    fun `parse ERR with memory error code`() {
        val result = JniResultParser.parse("ERR:3:Failed to allocate input grid")
        assertTrue(result is JniResult.Error)
    ...
    fun `parse ERR with colons in message preserves full message`() {
        val result = JniResultParser.parse("ERR:2:Error: value was: 42")
        assertTrue(result is JniResult.Error)
    ...
    fun `toGenerationFailure maps INVALID_PARAMS correctly`() {
        val jniResult = JniResult.Error(JniErrorCode.INVALID_PARAMS, "Size must be 3-9 for Classik profiles")
        ...
        assertEquals("Size must be 3-9 for Classik profiles", failure.message)
    }

    @Test
    fun `toGenerationFailure maps INVALID_MODES to InvalidModes`() {
        val jniResult = JniResult.Error(JniErrorCode.INVALID_MODES, "Incompatible modes")
        val failure = JniResultParser.toGenerationFailure(jniResult)

        assertTrue(failure is PuzzleGenerationResult.Failure.InvalidModes)
    }

    @Test
    fun `toGenerationFailure maps SIZE_LIMIT to SizeLimit`() {
        val jniResult = JniResult.Error(JniErrorCode.SIZE_LIMIT, "Size limit exceeded")
        val failure = JniResultParser.toGenerationFailure(jniResult)

        assertTrue(failure is PuzzleGenerationResult.Failure.SizeLimit)
    }
    ...
    fun `JniErrorCode fromCode returns correct enum`() {
        ...
        assertEquals(JniErrorCode.GENERATION_FAIL, JniErrorCode.fromCode(2))
        assertEquals(JniErrorCode.MEMORY, JniErrorCode.fromCode(3))
        assertEquals(JniErrorCode.INVALID_PARAMS, JniErrorCode.fromCode(4))
        assertEquals(JniErrorCode.CLUE_GENERATION, JniErrorCode.fromCode(5))
        assertEquals(JniErrorCode.INVALID_MODES, JniErrorCode.fromCode(6))
        assertEquals(JniErrorCode.SIZE_LIMIT, JniErrorCode.fromCode(7))
    }
