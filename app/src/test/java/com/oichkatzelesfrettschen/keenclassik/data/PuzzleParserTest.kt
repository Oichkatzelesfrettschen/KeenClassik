...
class PuzzleParserTest {
    ...
    @Test
    fun `parse valid 4x4 puzzle with standard operations`() {
        // 4x4 with 4 zones using standard operations
        val zoneIndices = "00,00,01,01,00,02,02,01,03,03,03,03,03,03,03,03"
        val zoneDefs = "a00010,m00024,s00002,d00002"
    ...
    @Test
    fun `zone values at 4-digit cap parse correctly`() {
        // Zone values are 5-digit fields with a strict 4-digit cap (<= 9999).
        val payload = "00,00,00,00,00,00,00,00,00;a09999,123456789"
        val result = PuzzleParser.parse(payload, 3)
        assertTrue(result is ParseResult.Success)

        val zone = (result as ParseResult.Success).puzzle.zones[0]
        assertEquals(9999, zone.targetValue)
    }

    @Test
    fun `zone values above 4-digit cap return failure`() {
        val payload = "00,00,00,00,00,00,00,00,00;a10000,123456789"
        val result = PuzzleParser.parse(payload, 3)
        assertTrue(result is ParseResult.Failure)
        assertTrue((result as ParseResult.Failure).message.contains("Zone value exceeds cap"))
    }
}
