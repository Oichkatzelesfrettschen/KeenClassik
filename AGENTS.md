# KeenClassik Agent Guide

This file holds the rules and orientation for every agent and contributor working in this repository.

## Instruction source

`AGENTS.md` is the root instruction file for KeenClassik and owns its rules. Every agent and contributor reads it directly. `CLAUDE.md` is a tracked, repository-relative symbolic link to `AGENTS.md`, so Claude Code receives the canonical rules through the same bytes and the body lives in one place. A tool that requires a differently named loader references this file rather than copying doctrine that can drift.
...
- `core/`: shared Kotlin models and configuration helpers.
...
- `./gradlew testDebugUnitTest`: run JUnit/Robolectric unit tests.
- `./gradlew connectedDebugAndroidTest`: run instrumented tests on device/emulator.
- `./gradlew lintDebug`: run Android Lint (warnings are errors).
- `make build` / `make test` / `make lint`: Makefile wrappers for common Gradle tasks.
- `make tools`: build host-side native tools for perf/analysis.
Prereqs: JDK 21, Android SDK (API 36), Android NDK 27.x, CMake 3.22+.
...
- JNI naming: `Java_com_oichkatzelesfrettschen_keenclassik_<Class>_<Method>`.
...
- Example: `./gradlew testDebugUnitTest --tests "*PuzzleParser*"`.
