...
- Gradle: `./gradlew perfHostBuild`, `perfHostRecord`, `perfHostFlamegraph`,
  ...
  `./gradlew connectedClassikDebugAndroidTest -PkeenBenchmark` (benchmarks)
...
- Wrapper: `scripts/run_android_tests.sh --ui|--bench --task=connectedClassikDebugAndroidTest`.
...
- `adb shell dumpsys gfxinfo com.oichkatzelesfrettschen.keenclassik.classik` for frame stats
...
- Heaptrack (manual): `heaptrack ./build/host/latin_gen_opt` then `heaptrack_gui heaptrack.*.gz`.
...
- Android PGO:
  - Build instrumented: `scripts/perf/gradle_native_instrument.sh --pgo=generate --lto -- assembleDebug`
  - Run and collect: `scripts/perf/android_pgo.sh --package=com.oichkatzelesfrettschen.keenclassik.classik --run="shell am start -n com.oichkatzelesfrettschen.keenclassik.classik/.KeenActivity"`
