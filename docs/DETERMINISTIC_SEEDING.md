...
# Deterministic Seeding in Keen Classik
...
Keen Classik supports deterministic puzzle generation where the same seed value
...
```
keenclassik://puzzle?seed=1735170000000&size=6&diff=2
```
...
viewModel.startNewGame(size = 5, diff = 2, multOnly = 0, seed = bugSeed, gameMode = GameMode.STANDARD)
```
